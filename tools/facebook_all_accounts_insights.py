...
async def get_all_accounts_insights(
    ...
    Retorna resumo com: nome da conta, status, gastos totais, resultados e CPR de cada conta.
    """
    try:
        ...
        if accounts_with_data:
            ...
            for idx, acc in enumerate(accounts_with_data, 1):
                ...
                result += f"   🎯 Resultados: {acc['results']:,}\n"
                result += f"   📊 CPR: R$ {acc['cpr']:.2f}\n"
                result += f"   🆔 `{acc['id']}`\n\n"
            
            result += "━━━━━━━━━━━━━━━━━━━\n"
            result += f"💵 *Total Investido:* R$ {total_spend_all:.2f}\n"
            result += f"🎯 *Total Resultados:* {total_results_all:,}\n"
